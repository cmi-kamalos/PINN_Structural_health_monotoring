...
def plot_discovery(model, scale = 1):
    ...
    plt.ylabel(model.domain.domain_ids[0])
    plt.show()
def plot_solution_Beam(model, domain, ub, lb, Exact_u=None, u_transpose=False):
    """
    Plot a 1D solution Domain
    Arguments
    ---------
    ...
    u_pred, f_u_pred = model.predict(X_star)
    if u_transpose:
        U_pred = griddata(X_star, u_pred.T.flatten(), (X, Y), method='cubic')
    else:
...
#     ax.set_xlabel('t')
    ...
    ax.set_xlim([-1.1,1.1])
    ax.set_ylim([-1.1,1.1])
    ax.set_title('s = %.2f' % (domain[1][2*len_]), fontsize = 10)
    ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.3), ncol=5, frameon=False)
    ...
    ax.set_xlim([-1.1,1.1])
    ax.set_ylim([-1.1,1.1])
    ax.set_title('s = %.2f' % (domain[1][3*len_]), fontsize = 10)
    plt.savefig("beam_pinn.png")
    plt.show()
